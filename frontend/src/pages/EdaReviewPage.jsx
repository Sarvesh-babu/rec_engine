...
import { getEda, startTraining } from "../api";
    ...
    <div className="min-h-screen text-slate-900">
      <header className="border-b border-sky-100/80 bg-white/70 backdrop-blur px-6 py-4 flex items-center justify-between shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500">Check the EDA below, then start training.</p>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        {run?.status === "eda_ready" && eda && (
          <div className="rounded-md border border-sky-200 bg-white/80 p-4 shadow-sm flex items-center justify-between gap-4">
            <p className="text-sm text-slate-600">Data looks good? Train the recommendation models next.</p>
            <button
              onClick={handleStart}
              disabled={starting}
              className="rounded-md bg-gradient-to-r from-sky-600 to-cyan-500 hover:from-sky-500 hover:to-cyan-400 text-white disabled:opacity-40 px-4 py-2 text-sm font-medium shadow-sm whitespace-nowrap"
            >
